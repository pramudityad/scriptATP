...
# context IPASO
	...
	if crt.Screen.ReadString('Host'):
		...
		pass
	if crt.Screen.ReadString('[Inband-IPaso]'):
		n12 = crt.Screen.CurrentRow
		result2 = crt.Screen.Get(n12, 1, n12, 15)	
		pass
	i_InbandIPaso = n12
	d_InbandIPaso = 1
	impacted_InbandIPaso = []
	while i_InbandIPaso > n11:		
		screenrow = crt.Screen.CurrentRow - d_InbandIPaso
		result = crt.Screen.Get(screenrow, 1, screenrow, 15)
		d_InbandIPaso += 1
		i_InbandIPaso -= 1
		impacted_InbandIPaso.append(result)	
	for ip_IPaso in impacted_InbandIPaso:
